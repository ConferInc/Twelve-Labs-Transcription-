...
def upload_assets():
    ...
    print("[REMOTE] Fetching verifiable state from TwelveLabs...")
    remote_assets = {}
    try:
        ...
        print(f"[REMOTE] Found {len(remote_assets)} assets in TwelveLabs account.")
    except Exception as e:
        print(f"[WARN] Could not fetch remote assets: {e}")
    ...
    print(f"[START] Found {len(chunk_files)} chunks to process...")
    ...
    for file_path in chunk_files:
        ...
        if filename in remote_assets:
            print(f"   [OK] {filename} exists remotely ({remote_assets[filename]})")
        ...
        # If not, upload it
        print(f"   [UPLOAD]  Uploading {filename}...", end="", flush=True)
        try:
    ...
    print(f"\n[OK] Sync Complete. {new_uploads_count} new files uploaded. DB synced with Remote.")
